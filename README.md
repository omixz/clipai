...
- Free users are capped at `FREE_LIMIT` (default 1) video, tracked via a cookie (`clipai_cid`) in `usage.json`. This is fine for an MVP but not abuse-proof — clearing cookies resets the counter, and on a host with no persistent disk (Render's free tier included) the whole file is wiped on every redeploy or spin-down/spin-up. Low stakes for a free counter; real accounts are the next step before this scales.

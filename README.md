...
- `config.py` holds every setting you need to fill in — payment method credentials, AdSense publisher ID, per-tier plan limits, max upload size. Every placeholder is marked `REPLACE_ME` / `REPLACE-ME`.
...
- **Pro/Pro Plus tier is deliberately NOT stored in that same file, only the video count is.** Billing went through Stripe, then Lemon Squeezy, then PayPal/bank transfer/NOWPayments, before landing back on Stripe as the primary/only path (see `billing_lib.py`'s module docstring for why the middle two didn't work out). `/create-checkout-session[-plus]` picks the active method via `_checkout_methods()`/`_checkout_via()` in `app.py`: **when Stripe is configured (`STRIPE_SECRET_KEY`/`STRIPE_PRICE_ID_PRO`/`STRIPE_PRICE_ID_PLUS` all set), it's the only option** — bank/crypto/PayPal below go dormant rather than showing a picker. None of these trust a local "is this user Pro" flag — that would get wiped by the same disk-persistence issue above, and unlike a video counter, losing a paying customer's tier on every restart is a real problem, not a minor one.
  - **Stripe** (`STRIPE_*` env vars, `stripe_lib.py`) — Checkout Session → subscription; every tier check calls Stripe's API live for that customer's subscription status (`get_subscription`), so a cancelled subscription stops granting Pro the moment Stripe's own status says so. Also the only path with a real self-serve billing portal (`/billing-portal`).
  - **Bank transfer** (`BANK_*` env vars) — manual, fallback only: the customer sees your account details and a reference code, and an admin grants Pro via `/admin/billing` once a matching deposit lands. No processor, no ID verification, an Australian BSB + account number is normal to publish (can't be used to pull money out, unlike US ACH). Requires an account (bank deposits need something to manually match against).
  - **NOWPayments** (`NOWPAYMENTS_*` env vars, crypto) — fallback only: a webhook grants Pro the instant payment confirms, no admin click needed. Signup is just an email at nowpayments.io.
  - **PayPal** (`PAYPAL_*` env vars, recurring subscriptions via `billing_lib.py`) — fallback only, same live-check pattern as Stripe.
...
1. **Stripe** (primary/only path once set — see the breakdown above): create a product with a recurring Price for Pro and one for Pro Plus (Dashboard > Product catalog), copy each Price id (`price_...`) into `STRIPE_PRICE_ID_PRO`/`STRIPE_PRICE_ID_PLUS`, copy your secret key (`sk_live_...`, or a restricted `rk_live_...` key with Checkout Sessions/Billing Portal Sessions write + Subscriptions read) into `STRIPE_SECRET_KEY`, and add a webhook endpoint at `{SITE_URL}/stripe/webhook` listening for `checkout.session.completed` and `customer.subscription.deleted` — copy its signing secret into `STRIPE_WEBHOOK_SECRET`. Test with `sk_test_...`/sandbox mode first.
   - **Fallback methods** (only relevant if you ever unset Stripe): bank transfer needs `BANK_NAME`/`BANK_ACCOUNT_NAME`/`BANK_ACCOUNT_NUMBER`/`BANK_BSB` (optionally `BANK_PAYID`), no signup; NOWPayments needs an account at [nowpayments.io](https://nowpayments.io) for `NOWPAYMENTS_API_KEY`/`NOWPAYMENTS_IPN_SECRET`; PayPal needs an app at [developer.paypal.com](https://developer.paypal.com) for `PAYPAL_CLIENT_ID`/`PAYPAL_CLIENT_SECRET`/`PAYPAL_PLAN_ID_PRO`/`PAYPAL_PLAN_ID_PLUS`/`PAYPAL_WEBHOOK_ID`.

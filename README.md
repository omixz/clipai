...
- **Pro/Pro Plus tier is deliberately NOT stored in that same file, only the video count is.** Billing went through Stripe, then Lemon Squeezy, before landing on the current setup — neither of the first two's identity verification worked out (see `billing_lib.py`'s module docstring). Today there are three payment methods, and `/create-checkout-session[-plus]` picks between whichever are configured (`_checkout_methods()`/`_checkout_via()` in `app.py`): if only one is set up, checkout goes straight there; if more than one is, the customer picks from a page. None of them trust a local "is this user Pro" flag — that would get wiped by the same disk-persistence issue above, and unlike a video counter, losing a paying customer's tier on every restart is a real problem, not a minor one.
  - **Bank transfer** (`BANK_*` env vars) — manual: the customer sees your account details and a reference code, and an admin grants Pro via `/admin/billing` once a matching deposit lands. No processor, no ID verification, an Australian BSB + account number is normal to publish (can't be used to pull money out, unlike US ACH). Requires an account (bank deposits need something to manually match against).
  - **NOWPayments** (`NOWPAYMENTS_*` env vars, crypto) — the only fully automatic path: a webhook grants Pro the instant payment confirms, no admin click needed. Signup is just an email at nowpayments.io.
  - **PayPal** (`PAYPAL_*` env vars, recurring subscriptions via `billing_lib.py`) — every tier check calls PayPal's API live for that customer's subscription status (`get_subscription`), so a cancelled/expired subscription stops granting Pro the moment PayPal's own status says so, the same "never trust local storage" property Stripe/Lemon Squeezy were built around.
...
1. **Payments** (pick one or more — see the breakdown above):
   - **Bank transfer**: put your account details in `BANK_NAME`/`BANK_ACCOUNT_NAME`/`BANK_ACCOUNT_NUMBER`/`BANK_BSB` (optionally `BANK_PAYID`). No signup needed.
   - **NOWPayments**: sign up at [nowpayments.io](https://nowpayments.io), add a payout wallet, and copy the API key into `NOWPAYMENTS_API_KEY`; grab the IPN secret from Store Settings > IPN into `NOWPAYMENTS_IPN_SECRET`.
   - **PayPal**: create an app at [developer.paypal.com](https://developer.paypal.com) > Apps & Credentials, copy the client ID/secret into `PAYPAL_CLIENT_ID`/`PAYPAL_CLIENT_SECRET`, create the two recurring plans (Pro/Pro Plus) and put their IDs in `PAYPAL_PLAN_ID_PRO`/`PAYPAL_PLAN_ID_PLUS`, and add a webhook (Apps & Credentials > your app > Webhooks) for `PAYPAL_WEBHOOK_ID`. Test against sandbox first (`PAYPAL_MODE=sandbox`) before flipping to `live`.
